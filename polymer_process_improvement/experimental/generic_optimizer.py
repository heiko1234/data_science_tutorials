...
from polymer_process_improvement.source.crisisTeamData_analysis import create_df_testing
...
# objective function
def objective(x):
	return x[0]**2.0 + x[1]**2.0 - x[0]




# objective function
def objective(target, x):
    M_per=x[0]
    Xf=x[1]
    SA=x[2]
    idata = create_df_testing(M_per, Xf, SA)
    modeloutput = imodel.predict(idata)
...
# genetic algorithm
def genetic_algorithm(objective, target, break_accuracy, bounds, n_bits, n_iter, n_pop, r_cross, r_mut):
    # initial population of random bitstring
    pop = [randint(0, 2, n_bits*len(bounds)).tolist() for _ in range(n_pop)]
    # keep track of best solution
...
# perform the genetic algorithm search
best, score = genetic_algorithm(objective, target, break_accuracy, bounds, n_bits, n_iter, n_pop, r_cross, r_mut)

decoded = decode(bounds, n_bits, best)
decoded
rounded_values = [round(element,3) for element in decoded]
rounded_values
